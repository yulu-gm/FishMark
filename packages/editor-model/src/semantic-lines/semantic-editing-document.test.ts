...
// The canonical model document owns line geometry and the canonical tree owns the semantic roles.
const createEditingDocument = (source: string) => {
  const snapshot = createEditorDerivedSnapshotFromCache(createDocumentStructureCache(source));

  return createSemanticEditingDocument(snapshot.document, snapshot);
};

describe("createSemanticEditingDocument", () => {
  ...
  it("represents mixed text, blank, and whitespace source lines", () => {
    ...
    const document = createEditingDocument(source);
  ...
  it("maps heading and structural blank separator overlays without faking blocks", () => {
    ...
    expect(document.semanticLineMap.lines.map((line) => ({
      ...
      { lineNumber: 1, role: "heading" },
      { lineNumber: 2, role: "structural-separator" },
      { lineNumber: 3, role: "paragraph" }
    ]);
  });
  ...
  it("marks styled block leading separators without treating whitespace lines as structural", () => {
    ...
    expect(document.semanticLineMap.lines.map((line) => ({
      ...
      { lineNumber: 1, role: "paragraph" },
      { lineNumber: 2, role: "extra-blank" },
      { lineNumber: 3, role: "structural-separator" },
      { lineNumber: 4, role: "table-source" },
      { lineNumber: 5, role: "table-source" }
    ]);
  });
