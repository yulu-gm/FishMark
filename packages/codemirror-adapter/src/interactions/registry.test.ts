...
import { createActiveBlockState, deriveTableCursorState, type ActiveBlockState } from "@fishmark/editor-model";
...
function createVerticalNavigationHarness(source: string, anchor: number): {
  ...
  const activeState = createActiveBlockState(snapshot, selection);

  activeState.tableCursor = deriveTableCursorState(snapshot, selection, null);
