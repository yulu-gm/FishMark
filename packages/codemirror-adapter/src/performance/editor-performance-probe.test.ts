...
import { createLongMarkdownFixture } from "@fishmark/editor-model";
import {
...
describe("measureEditorPerformanceProbe", () => {
  it("records honest open, edit, selection, and ordered-list operation evidence", () => {
    ...
    // The canonical structure cache owns parsing, so the legacy rich-document entry point is
    // untouched and the open cost is reported by the real parser counters.
    expect(open?.parserEntries.parseMarkdownDocument).toBe(0);
    expect(open?.counters.fullParse).toBeGreaterThan(0);
