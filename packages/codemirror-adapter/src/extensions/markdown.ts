...
import { createEditorDerivedSnapshotFromCache, planNormalizeOrderedListScopes, planPointerSelection, planTableUpdateCell, reselectEditorSemanticContext } from "@fishmark/editor-model";
...
import { runMarkdownHardBreak } from "../codemirror-markdown-commands";
import {
...
export function createFishMarkMarkdownExtensions(
  ...
  // The empty document boundary has no editor state yet, so it builds its own canonical cache.
  // Every later read reuses the view's structure cache instead of parsing the source again.
  const initialEditorDerivedState = createEditorDerivedState({
    snapshot: createEditorDerivedSnapshotFromCache(createDocumentStructureCache("")),
  ...
  const readStateSnapshot = (state: EditorState): EditorDerivedSnapshot =>
    createEditorDerivedSnapshotFromCache(readEditorStructureCache(state));
  ...
  const notifyActiveBlockChange = (nextState: ActiveBlockState, force = false) => {
    const didChange =
      ...
      runtime.activeBlockState.activeRootNodeId !== nextState.activeRootNodeId ||
      runtime.activeBlockState.snapshot.tree !== nextState.snapshot.tree ||
  ...
  const jumpToFootnoteDefinition = (view: EditorView, identifier: string | null | undefined): boolean => {
    ...
    const footnoteDefinition = readEditorStructureCache(view.state).tree.footnoteDefinitions.get(identifier);
  ...
  return [
    ...
    EditorState.transactionFilter.of((transaction) => {
      ...
      if (
        ...
        const snapshot = createEditorDerivedSnapshotFromCache(readEditorStructureCache(effectiveState));
...
function createDetachedListBlankLineInsertTransaction(
  transaction: Transaction,
  readStateSnapshot: (state: EditorState) => EditorDerivedSnapshot
  ...
  if (!isWhitespaceLineAfterList(readStateSnapshot(transaction.startState), source, line.from)) {
    return null;
  }
...
// A detached-list blank line is a whitespace-only line whose preceding root-level block is a list
// with nothing but whitespace between that list and this line.
function isWhitespaceLineAfterList(
  snapshot: EditorDerivedSnapshot,
  source: string,
  lineStart: number
): boolean {
  let previousBlock: MarkdownNode | null = null;

  for (const node of childrenOf(snapshot.tree.root)) {
    if (node.source.startOffset > lineStart) {
      break;
    }

    if (node.source.endOffset <= lineStart) {
      previousBlock = node;
    }
  }
  ...
  const gap = source.slice(previousBlock.source.endOffset, lineStart);

  return /^[\r\n \t]*$/u.test(gap);
}

function findLinkAtOffset(snapshot: EditorDerivedSnapshot, offset: number): InlineLink | null {
  const node = snapshot.nodeAt(offset);

  return node !== null && isMarkdownLeafNode(node) ? findLinkInInline(node.inline, offset) : null;
}
