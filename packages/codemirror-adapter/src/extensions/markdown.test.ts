    ...
    state: EditorState.create({
      ...
      extensions: [editorStructureObserver.of(options.structureObserver ?? {}), ...createFishMarkMarkdownExtensions({
        ...
        onActiveBlockChange: (state) => {
          options.onActiveBlockChange?.(state.activeKind, state.selection.anchor);
        },
...
describe("createFishMarkMarkdownExtensions", () => {
  ...
  it("uses the canonical incremental tree instead of re-parsing the whole document", () => {
    const source = "# Title\n\nParagraph";
    const fullParser = vi.fn();
    const incrementalNodes: Array<number | null> = [];
    const { view, destroy } = createHarness({
      source,
      structureObserver: {
        instrumentation: { onFullDocumentParse: fullParser },
        onUpdate: (stats) => { incrementalNodes.push(stats === null ? null : stats.reparsedNodes); }
      }
    });
    ...
    expect(incrementalNodes.length).toBeGreaterThan(0);
    expect(incrementalNodes.some((nodes) => nodes !== null)).toBe(true);

    destroy();
  });

  it("reuses the canonical structure cache for selection-only updates", () => {
    const source = "# Title\n\nParagraph";
    const fullParser = vi.fn();
    const cacheHits: Array<number | null> = [];
    const { view, destroy } = createHarness({
      source,
      structureObserver: {
        instrumentation: { onFullDocumentParse: fullParser },
        onUpdate: (stats) => { cacheHits.push(stats === null ? null : stats.reparsedNodes); }
      }
    });
    ...
    expect(cacheHits.every((nodes) => nodes === null)).toBe(true);

    destroy();
  });

  it("does not normalize ordered lists for non-list document changes", () => {
    const source = "Paragraph";
    const onContentChange = vi.fn();
    const { view, destroy } = createHarness({ source, onContentChange });
    ...
    // One content change means the normalization planner never rewrote the document again.
