...
import { createActiveBlockState, createEditorDerivedState } from "@fishmark/editor-model";
import { createBlockDecorations as buildDecorations } from "./block-decorations";
import { getInactiveBlockquoteLines, getInactiveCodeFenceLines } from "@fishmark/editor-model";
...
function createBlockDecorations(options: Omit<Parameters<typeof buildDecorations>[0], "snapshot">) {
  return buildDecorations({ ...options, snapshot: createEditorDerivedSnapshotFromCache(createDocumentStructureCache(options.source)) });
}
...
const createDecorationsForSelection = (
  ...
  const activeState = createActiveBlockState(snapshotOf(source), selection);
...
describe("createBlockDecorations", () => {
  ...
  it("adds blockquote container metadata to quote-internal block math previews", () => {
    const quotedSource = ["> $$", "> x = 1", "> $$"].join("\n");
    const quoted = createBlockDecorations({
      activeBlockState: createActiveBlockState(snapshotOf(quotedSource), { anchor: 0, head: 0 }),
      hasEditorFocus: false,
      source: quotedSource
    });
    const rootSource = ["$$", "x = 1", "$$", "", "Tail"].join("\n");
    const root = createBlockDecorations({
      activeBlockState: createActiveBlockState(snapshotOf(rootSource), { anchor: rootSource.indexOf("Tail"), head: rootSource.indexOf("Tail") }),
      hasEditorFocus: false,
      source: rootSource
    });
    ...
    globalWithDocument.document = dom.window.document;
    try {
      const fakeView = { dom: dom.window.document.createElement("div"), requestMeasure: () => undefined };
      const quotedRoot = findWidget(quotedSource, quoted.decorationSet, "MathPreviewWidget")?.toDOM(fakeView);
      const plainRoot = findWidget(rootSource, root.decorationSet, "MathPreviewWidget")?.toDOM(fakeView);

      expect(quotedRoot?.classList.contains("cm-math-preview-block")).toBe(true);
      expect(quotedRoot?.classList.contains("cm-math-preview-blockquote")).toBe(true);
      expect(plainRoot?.classList.contains("cm-math-preview-blockquote")).toBe(false);
    } finally {
      if (previousDocument) {
        globalWithDocument.document = previousDocument;
      } else {
  ...
  it("keeps a point inline widget on the active line of a container-nested paragraph", () => {
    const source = ["- Alpha<br>Beta", "", "Tail"].join("\n");
    const head = source.indexOf("Alpha");
    const result = createBlockDecorations({
      activeBlockState: createActiveBlockState(snapshotOf(source), { anchor: head, head }),
      hasEditorFocus: true,
      source
    });
    const breakEnd = source.indexOf("<br>") + "<br>".length;
  ...
  it("does not duplicate a replaced inline atom that spans a soft line break", () => {
    const singleLine = ["- ![alt](hero.png)", "", "Tail"].join("\n");
    const softBreak = ["- ![alt", "  text](hero.png)", "", "Tail"].join("\n");

    const decorate = (source: string) => {
      const head = source.indexOf("Tail");
      const decorationSet = createBlockDecorations({
        activeBlockState: createActiveBlockState(snapshotOf(source), { anchor: head, head }),
        hasEditorFocus: true,
        source
      ...
      return {
        ranges: collectDecorations(source, decorationSet),
        widgets: collectWidgets(source, decorationSet)
      };
    };

    const single = decorate(singleLine);
    expect(single.widgets).toContainEqual({
      from: 2,
      to: 2 + "![alt](hero.png)".length,
      name: "MarkdownImagePreviewWidget"
    });

    const spanning = decorate(softBreak);
    // The canonical image node still spans the break, so the two lines must not each replace it
    // with their own preview widget.
    expectCoveredRangeClasses(spanning.ranges, softBreak.indexOf("["), softBreak.indexOf("[") + 1, [
  ...
  it("hides a nested heading marker only while its line is inactive", () => {
    ...
    expectExactRangeClasses(decorateFor(source.indexOf("Tail")), 0, 0, [
      "cm-inactive-blockquote cm-inactive-blockquote-depth-1 cm-inactive-blockquote-start cm-inactive-blockquote-end",
      "cm-inactive-heading cm-inactive-heading-depth-1"
    ]);
    expectExactRangeClasses(decorateFor(source.indexOf("Tail")), 2, 4, ["cm-inactive-heading-marker"]);
  ...
  it("derives inactive block decorations and a stable signature for non-active top-level blocks", () => {
    ...
    expect(ranges).toEqual([
      ...
      // The blockquote line owns the line box, so its leading is not overridden by the paragraph.
      {
        from: 27,
        to: 29,
  ...
  it("narrows an active quoted list continuation source prefix to its leading whitespace", () => {
    const source = ["- > quote", "  > continued", "", "Paragraph"].join("\n");
    const continuationStart = source.indexOf("  > continued");
    const activeState = createActiveBlockState(snapshotOf(source), {
      anchor: source.indexOf("continued"),
      head: source.indexOf("continued")
    });
    ...
    // Only the leading whitespace leaves the flow: the quote marker and its padding anchor stay in
    // it, which is what keeps the row from collapsing.
    expect(
      ranges.filter(
        (range) => range.from === continuationStart && range.className.includes("list-source-prefix")
      )
    ).toEqual([
      {
        from: continuationStart,
        to: continuationStart + 2,
        className: "cm-active-list-source-prefix",
        text: "  "
      }
    ]);
    expectExactRangeClasses(ranges, continuationStart, continuationStart, [
      "cm-active-list-continuation cm-active-list-unordered cm-active-list-depth-0",
      "cm-inactive-blockquote cm-inactive-blockquote-depth-1 cm-inactive-blockquote-end"
    ]);
    expectExactRangeClasses(ranges, continuationStart + 3, continuationStart + 4, [
      "cm-active-blockquote-padding-anchor"
    ]);
  });

  it("keeps the source prefix of an inactive quoted list continuation narrowed to its whitespace", () => {
    const source = ["- > quote", "  > continued", "", "Paragraph"].join("\n");
    const continuationStart = source.indexOf("  > continued");
    const activeState = createActiveBlockState(snapshotOf(source), {
    ...
    // The quote marker itself stays hidden by the blockquote marker mark, so the narrowed prefix
    // does not reveal it.
  ...
  it("renders inactive Mermaid code fences with preview widgets and restores source while active", () => {
    ...
    const inactiveState = createActiveBlockState(snapshotOf(source), {
      anchor: source.indexOf("Paragraph"),
      head: source.indexOf("Paragraph")
    });
    const activeState = createActiveBlockState(snapshotOf(source), {
