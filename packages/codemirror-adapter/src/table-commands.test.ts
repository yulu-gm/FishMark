...
import { createSemanticCommandBindings, planSemanticTab, runSemanticCommand } from "./semantic-keypress";
import {
...
const createHarness = (doc: string, anchor: number) => {
  ...
  const activeState = createActiveBlockState(snapshot, { anchor, head: anchor });
  const activeStateWithCursor = {
    ...activeState,
    tableCursor: deriveTableCursorState(snapshot, { anchor, head: anchor }, null)
  };
...
describe("table commands", () => {
  it("dispatches table next-cell navigation from Tab before list indent fallback", () => {
    ...
    // `runMarkdownTab(view, state)` in editor-core is literally
    // `runSemanticCommand(view, planSemanticTab)` (it discards the state), so the moved test
    // exercises the identical production call instead of importing editor-core.
    expect(runSemanticCommand(harness.view, planSemanticTab)).toBe(true);
  ...
  it("moves up and down within the same table column", () => {
    ...
    const nextSnapshot = snapshotFor(harness.view.state.doc.toString());
    const nextActiveState = createActiveBlockState(
      nextSnapshot,
      {
  ...
  it("moves left and right across cells only at cell boundaries", () => {
    ...
    const startSnapshot = snapshotFor(harness.view.state.doc.toString());
    const startActiveState = createActiveBlockState(
      startSnapshot,
    ...
    const endSnapshot = snapshotFor(harness.view.state.doc.toString());
    const endActiveState = createActiveBlockState(
      endSnapshot,
