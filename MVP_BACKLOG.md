...
## Editor Foundation 当前执行任务

原有 RF 任务分解和依赖由本 backlog 引用 `docs/refactor/editor-foundation/roadmap.md`，状态与历史证据集中在该目录的 `progress.md`；不另建平行版本计划。

### RF-HARDEN-001 复审阻断修复与后续计划调整

状态：ACCEPTED

目标：修复 `1b4c329` 复审发现的数据恢复、增量缓存和桥接版本缺陷；一个工作 agent 实现、父 agent 独立验收。后续重构专注可维护与可扩展、编辑与交互体验、性能三项结果。

验收：
- 新会话编辑有可重放基线，日志失败不假确认，恢复追加/压缩/退出顺序有真实文件及进程边界证据。
- 删除空行、未闭合围栏、引用目标变化的增量结果等价于完整解析；普通文本快路径无隐藏全文解析；不能证明局部安全时使用明确记录原因的正确回退。
- 过期语义计划不提交，多范围编辑不重置版本，选区读取复用同一文档派生快照。
- build、lint、typecheck、相关与全量测试通过，独立复现原审查断言；不把完整 IME、真实 20k 输入到绘制性能或 RF-506 切换记为本轮已完成。
- 计划明确 RF-HARDEN-001 → RF-601 → RF-506 → RF-701 → RF-602/603/604 的前置关系，并复用已存在的 client/queue。

执行切片：
- [x] 恢复生命周期与持久化回归修复。
- [x] 增量正确性、普通输入快路径与诚实计数。
- [x] 桥接版本和派生快照修复。
- [x] 按用户三项重点调整后续路线及前置验收。
- [x] 父 agent 独立验收，更新证据与任务总结。
