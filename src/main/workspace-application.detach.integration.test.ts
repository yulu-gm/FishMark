...
function createWorkspaceDetachForTest<TWindow>(dependencies: {
  workspace: Parameters<typeof createWorkspaceDetach<TWindow>>[0]["workspace"];
  tabTransfer: Parameters<typeof createWorkspaceDetach<TWindow>>[0]["tabTransfer"];
  openWindow: () => TWindow;
  scheduleReadyTimeout: (listener: () => void) => () => void;
  lifecycle: Omit<
    Parameters<typeof createWorkspaceDetach<TWindow>>[0]["windowLifecycle"],
    "openWindow" | "scheduleReadyTimeout"
  >;
}) {
  const { openWindow, scheduleReadyTimeout, lifecycle, ...rest } = dependencies;
  return createWorkspaceDetach<TWindow>({
    ...rest,
    windowLifecycle: { openWindow, scheduleReadyTimeout, ...lifecycle }
  });
}
...
function createTabTransfer(
  ...
  return createWorkspaceTabTransfer({
    workspace,
...
describe("workspace detach use case", () => {
  it.each([
    {
      name: "timeout",
    ...
    async ({ cancel, message }) => {
      ...
      const application = createWorkspaceDetachForTest({
        workspace,
        tabTransfer: createWorkspaceTabTransfer({
          workspace,
          documentOperations
        }),
        openWindow: () => window,
