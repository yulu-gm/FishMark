...
9. 使用 rail 中的 `Row Above`、`Row Below`、`Column Left`、`Column Right`、`Delete Row`、`Delete Column`、`Delete Table`，尤其验证插入空白列后表格仍保持渲染态。
...
- rail 中的增删行列和删除整表都能生效，且焦点保持在逻辑上合理的单元格
