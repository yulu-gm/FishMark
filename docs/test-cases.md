...
- CodeMirror 只由 exact exceptions 放行；新增 importer/specifier、wildcard 或 stale debt 均失败，exact exception identity 不因路径规范化而扩大。static/dynamic/re-export/import type/import-equals/literal `require` 进入同一 guard；透明包装的 literal module argument 仍是依赖证据，shadowed literal `require` 保守失败，注释、普通字符串和非 literal call 不伪报。
- 所有 scanner 已支持的直接 runtime micromark module form，以及由它确认的 namespace/`parse` 同步变量别名、namespace destructuring 和静态 `.parse` / `["parse"]` extraction，都必须送入同一 document-site registry；await/parentheses/`as`/type assertion/`satisfies`/non-null 不能包住 module literal 绕过。仅 import、非 `.document()` parser、其他包同名 API、micromark 非 `parse` 属性、未调用的 alias、注释/字符串不得产生 micromark site finding；assignment、promise 与 callback flow 不在本轮声明的覆盖范围。
