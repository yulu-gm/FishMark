...
## RF-506 生产语义切换回归（2026-09-19）

- 实际 controller 的键盘、格式快捷键、表格工具栏/widget 都调用 model plan；同一操作仅产生一个最终 change frame，撤销重做和原生输入共用 frame owner。
- 切换 tab、相同文本重新加载、销毁 view 后，旧 session 计划不能修改或重复发送新文档；相同 revision 的另一个 view 计划也应拒绝。
- 引用和列表混合嵌套中的 Enter、Backspace、Tab/Shift-Tab、代码/公式围栏空行与表格出口保留精确源码和选区。真实检查点读取当前 canonical cache，同时独立测量 DOM；不能用缺失子节点的旧投影替代实际语义。
- 语义导航、结构编辑及普通计划保留 scrollIntoView；组合输入期间冻结结构操作，结束后仅刷新一次；原生平台 IME 手测另记。
- 5k/20k 文档分别测冷启动、暖输入、选区移动；公开结构 observer 必须报告实际扫描和回退，普通编辑不能隐含第二次全文解析。空行分隔的 2,000 段普通文本需保持线性表格候选扫描量，紧凑/宽松表格识别不能退化。
...
4. 在空行输入 `>` 后直接输入 `quote`；另起一处空行输入 `>` 后把光标移动到其他段落；再另起一处空行输入 `>` 后直接按 `Enter`，再输入 `quote`。
5. 把光标放到 `> 第一段` 行末，按 `Enter`。
6. 把光标放到新生成的引用内空行，再按 `Enter`。
7. 输入 `> - item`，把光标放到行末按 `Enter`。
8. 输入 `> - parent\n>   - `，把光标放到空子项末尾按 `Enter`；再输入 `> 1. 111\n> 2. 333\n>   1. 222\n>     1. 1.1\n>     2. `，把光标放到最后一个空有序子项末尾按 `Enter`。
9. 输入 `> - parent\n> - `，把光标放到空顶级项末尾按 `Enter`。
...
- 顶层活动裸 `>` 在提交前保持普通可见源码，不显示 quote rail；继续输入正文、按 `Enter` 或把光标移出该行后才提交为引用块
- 已提交引用块内输入内层裸 `>` 时，活动行仍只显示父层引用样式并保留最后一个 `>` 作为普通可见源码；继续输入正文形成 `> >text` 后才提交为二层引用并隐藏完整 quote prefix，光标仍落在嵌套引用内容列
- 顶层裸 `>` 行尾按 `Enter` 也会提交为引用块并续出引用内编辑行，例如输入 `>` 后按 `Enter` 再输入 `quote` 得到 `> \n> quote`；引用内 `> >` 行尾按 `Enter` 同理提交为嵌套引用并得到 `> > \n> > nested`
- 光标移出裸 `>` 空引用行后，该行进入 inactive 引用渲染，marker 不可见但 quote rail 保持连续
- 光标不能进入引用块内部结构空行，例如 `> 11\n>\n> > 1` 中间的 `>` 行；尝试点击或选择该行时 selection 应停在相邻可编辑内容上
- 在引用块内部结构空行后的下一条引用内容行可见行首按 `Backspace`，会删除该结构空行，例如 `> 11\n>\n> > 1` 变为 `> 11\n> > 1`
- 引用块提交态激活时不显示原始 quote prefix，光标和正文起点停在引用内容列
...
- 引用内嵌套有序列表空子项按 Enter 升级到父列表时，编号和外部正文列表完全一致继续递增，例如 `>   1. 222\n>     1. 1.1\n>     2. ` 升级后得到 `>   1. 222\n>     1. 1.1\n>   2. `，不会重启为 `>   1. `
...
7. 在 synthetic repository 中分别新增同 importer 的新 CodeMirror package、新 importer 的既有 CodeMirror package、stale/wildcard exception、package rule 复用/错误 kind/sourcePath，以及 static literal `require` / import-equals / import type 的 forbidden/public-entry 穿透。再用 `.`, `..`、重复分隔符、反斜杠和大小写组合为同一目录复制 active rule 与 exact exceptions，确认 resolved repo-relative identity 仍拒绝 rule 复用。
8. 在未注册的 markdown-engine module 中分别用 import-equals namespace、literal `require` namespace/destructuring/property rename/direct call，以及 awaited dynamic import namespace/destructuring/property rename/direct call执行 `micromark.parse().document()`；继续覆盖 direct module `.parse` extraction、确认后的 namespace `.parse` extraction、namespace destructuring/rename、namespace/`parse` 同步别名链与静态 `["parse"]` extraction/call。对 dynamic import/literal require 的 module string 以及 literal `require` callee 逐项加入 parentheses、`as const`、type assertion、`satisfies`、non-null、`await` 和嵌套 callee-plus-argument wrappers，并保留 non-literal、其他包、property/global `require`、仅加载模块、非 document parser、micromark 非 `parse` property、alias 未调用 `.document()` 和注释/字符串的 negative cases。
...
- CodeMirror 只由 exact exceptions 放行；新增 importer/specifier、wildcard 或 stale debt 均失败，exact exception identity 不因路径规范化而扩大。static/dynamic/re-export/import type/import-equals/literal `require` 进入同一 guard；透明包装的 literal module argument 与 literal `require` callee 仍是依赖证据，shadowed literal `require` 保守失败，property/global `require`、注释、普通字符串和 non-literal call 不伪报。
- 所有 scanner 已支持的直接 runtime micromark module form，以及由它确认的 namespace/`parse` 同步变量别名、namespace destructuring 和静态 `.parse` / `["parse"]` extraction，都必须送入同一 document-site registry；await/parentheses/`as`/type assertion/`satisfies`/non-null 不能包住 module literal 或 literal `require` callee 绕过。仅 import、非 `.document()` parser、其他包同名 API、property/global `require`、micromark 非 `parse` 属性、未调用的 alias、注释/字符串不得产生 micromark site finding；assignment、promise 与 callback flow 不在本轮声明的覆盖范围。
