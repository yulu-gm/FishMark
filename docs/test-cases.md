...
6. 在 analyzer unit fixture 中分别移除一个 initial chunk map、写入非法 JSON、使用缺失/错误的 v3 `version`、缺失/非字符串/空/非法或不引用 source 的 `mappings`、越界 source 引用、缺失 `sourcesContent` 或不一致的 source/content 数量，再运行 `npm.cmd run test -- src/main/analyze-renderer-bundle.test.ts src/main/virtual-runtime-source-map.test.ts`。
...
- 任一适用 initial chunk 的 source map 缺失、JSON 非法、不是 Source Map v3、`mappings` 为空/不可解析/没有 source reference/引用越界，或 `sources` / `sourcesContent` 形状及对应内容不完整时，requested forbidden source-group rule 必须 `FAIL` 且进程以非零退出；不能因其他 chunk 有 map 或存在形式 JSON 而通过。
