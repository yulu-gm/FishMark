...
## Formal architecture P1 closure

The first formal review found one remaining compatibility owner in `editor-shell-state.ts`: optional `currentEditorContent` / `preserveActiveDocumentDraft` inputs could preserve a draft and suppress reload outside `WorkspaceRendererApplication`, and the same module retained an unused `getWorkspaceTabs()` export. Repository scans confirmed that no caller used either surface.

The shell-state boundary is now a pure canonical projector. `applyWorkspaceSnapshot(currentState, snapshot)` accepts no editor buffer or preservation option; active-tab/content identity alone controls the load revision. The merge helper, option type, reload bypass, dead export, and their exclusive import are deleted. The architecture source guard forbids all five obsolete tokens, keeping draft reconciliation exclusively behind the application identity/outbox/FIFO. The finding was reproduced RED and repaired GREEN; formal architecture re-review is required before any PASS verdict.
