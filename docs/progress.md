...
Editor Foundation 重构中的 `RF-101` 已进入 `DEV_DONE`，等待独立架构验收与任务验收。生产级 `@fishmark/workspace-domain` 现在是 workspace/window/tab/document session 的唯一 canonical state；main 持有唯一 live `WorkspaceState`，通过显式 mapper 输出 shared IPC DTO。dirty 由 revision equality 派生，精确 saved-text checkpoint 支持恢复到已保存内容即 clean。Draft 更新由调用 renderer 推导 expected owner，并在 domain 做 compare-and-set；旧窗口迟到同步既不能修改新 owner，也不会收到新 owner 的 projection。Save、Save As、reload、individual close 与 cross-window move/detach transfer 共享同一 per-tab FIFO transaction，owner 在 lease 内复验，owner/missing 结果显式失败。Reload revision race 通过唯一 `success(snapshot) | revision-stale` 结果硬切：stale 不应用 snapshot、不记录 recent、不清 external conflict，autosave 与手动保存保护继续生效。Detach 等待 target ready 时不占 lease；ready 后等待 shared lease 期间发生 timeout/close/load-fail 会同时拒绝 detach 与 ready、销毁 target，并保持 domain/last-focus/main binding 不变。独立 window-registration application 在 ready 前捕获真实 sender-owned `BrowserWindow` 对象和 ID，ready 后复验同一 live identity，随后才依次 register、bind、focus；无 sender ID fallback。Native close 分成 transport timeout、无计时 active confirmation、scope 结束后等待 COMPLETE 的独立 watchdog 三阶段，任何缺失都 fail closed。旧 `src/main/workspace-service.ts`、对应测试、compatibility facade、reload allow-stale 通道和双状态路径均不存在。

新鲜开发门禁：live registration 扩展 focus 10 文件 / 102 测试，editor-foundation 7 / 259，全量 Vitest 152 / 1,915；lint 0 error / 8 个既有 warning，typecheck、renderer/workspace-domain/Electron/CLI build、workspace-domain emitted runtime verifier 均通过；正式 editor behavior 为 121/121 cases、2,541/2,541 targets、0 unexpected/not-run。总重构进度仍为 2/38，M1 仍为 0/2，只有验收通过后才能标记 `RF-101 COMPLETE` 并开始 `RF-102`。
