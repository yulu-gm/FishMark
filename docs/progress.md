...
Editor Foundation 重构中的 `RF-101` 已进入 `DEV_DONE`，等待独立架构验收与任务验收。生产级 `@fishmark/workspace-domain` 现在是 workspace/window/tab/document session 的唯一 canonical state；main 持有唯一 live `WorkspaceState`，通过显式 mapper 输出原有 shared IPC DTO。dirty 由 revision equality 派生，精确 saved-text checkpoint 支持恢复到已保存内容即 clean。Draft 更新由调用 renderer 推导 expected owner，并在 domain 做 compare-and-set；旧窗口迟到同步既不能修改新 owner，也不会收到新 owner 的 projection。Save、Save As、reload、individual close 与 cross-window move/detach transfer 共享同一 per-tab FIFO transaction，owner 在 lease 内复验，owner/missing 结果显式失败；reload 只对 revision-only race 保留更新后的 dirty projection。Detach 等待 target ready 时不占 lease，ready 后再与 IO 竞争并原子移动最新 canonical session。Native close 分成 transport timeout、无计时 active confirmation、scope 结束后等待 COMPLETE 的独立 watchdog 三阶段，任何缺失都 fail closed。旧 `src/main/workspace-service.ts`、对应测试、compatibility facade 和双状态路径均不存在。

新鲜开发门禁：architecture-fix focus 14 文件 / 142 测试，真实 shared-coordinator integration 12/12，editor-foundation 7 / 259，全量 Vitest 151 / 1,899；lint 0 error / 8 个既有 warning，typecheck、renderer/workspace-domain/Electron/CLI build、workspace-domain emitted runtime verifier 均通过；正式 editor behavior 为 121/121 cases、2,541/2,541 targets、0 unexpected/not-run。总重构进度仍为 2/38，M1 仍为 0/2，只有验收通过后才能标记 `RF-101 COMPLETE` 并开始 `RF-102`。
