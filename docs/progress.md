...
| TASK-062 | 长文档打开性能优化 | TODO | 已基于 `tmp/complex_stress_test.md` 性能日志完成 intake：main 侧打开约 17.77ms，不是瓶颈；renderer 首次 `createState` 约 349.80ms，随后同内容重复 `replaceDocument -> createState` 约 266.10ms，`setDocumentPath.refreshDecorations` 约 111.00ms，outline / metrics 同步计算约 146.40ms / 229.20ms，打开期额外 6 次 derived state 重算累计约 561.70ms。下一步按 `docs/plans/2026-06-06-task-062-long-document-open-performance-intake.md` 执行。 |
