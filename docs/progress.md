...
Editor Foundation 重构中的 `RF-101` 已进入 `DEV_DONE`，等待独立架构验收与任务验收。生产级 `@fishmark/workspace-domain` 现在是 workspace/window/tab/document session 的唯一 canonical state；main 持有唯一 live `WorkspaceState`，通过显式 mapper 输出 shared IPC DTO。dirty 由 revision equality 派生，精确 saved-text checkpoint 支持恢复到已保存内容即 clean。Draft 更新和 reorder 都由调用 renderer 推导 expected owner；旧窗口迟到请求既不能修改新 owner，也不会收到新 owner 的 projection。Save、Save As、reload、individual close、reorder 与 cross-window move/detach transfer 共享同一 per-tab FIFO transaction，owner 在 lease 内复验，owner/missing 结果显式失败。Reload revision race 通过唯一 `success(snapshot) | revision-stale` 结果硬切：stale 不应用 snapshot、不记录 recent、不清 external conflict，autosave 与手动保存保护继续生效。Detach 等待 target ready 时不占 lease；同一 target 的并发 ready caller 共享一个 in-flight Promise，任何 caller 都不能提前 register。ready 后等待 shared lease 期间发生 timeout/close/load-fail 会用同一 authoritative error 拒绝 detach 与所有 ready caller、只销毁一次 target，并保持 domain/last-focus/main binding 不变。独立 window-registration application 在 ready 前捕获真实 sender-owned `BrowserWindow` 对象和 ID，ready 后复验同一 live identity，随后才依次 register、bind、focus；无 sender ID fallback。Native close 分成 transport timeout、无计时 active confirmation、scope 结束后等待 COMPLETE 的独立 watchdog 三阶段，任何缺失都 fail closed。旧 `src/main/workspace-service.ts`、对应测试、owner-blind reorder、compatibility facade、reload allow-stale 通道和双状态路径均不存在。

新鲜开发门禁：并发 ready/reorder ownership focus 10 文件 / 127 测试，editor-foundation 7 / 259，全量 Vitest 153 / 1,923；lint 0 error / 8 个既有 warning，typecheck、renderer/workspace-domain/Electron/CLI build、workspace-domain emitted runtime verifier 均通过；正式 editor behavior 为 121/121 cases、2,541/2,541 targets、0 unexpected/not-run。总重构进度仍为 2/38，M1 仍为 0/2，只有验收通过后才能标记 `RF-101 COMPLETE` 并开始 `RF-102`。
