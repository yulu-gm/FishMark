...
| TASK-038 | 跨平台打包 | DEV_IN_PROGRESS | 已接入基于 `electron-builder` 的 Windows 本地 `package:win` / `release:win` 与 macOS 本地 `package:mac` / `release:mac` / `release:mac:beta` 入口，并在打包前按需从 `assets/branding/fishmark_mark.svg` 生成 `light` / `dark` 两套 PNG 与 Windows `icon.ico`；Windows release 继续产出 NSIS installer 与 `latest.yml`，正式 macOS release 现已具备 arm64 `.dmg` / `.zip` / `latest-mac.yml` 构建与 GitHub Release 上传脚本，且会在发布前强制校验 Developer ID Application 签名材料与 Apple notarization 凭据；beta macOS release 可在无 Apple 凭据时发布 ad-hoc signed、未公证的 arm64 `.dmg` 到 `v<version>-mac-beta` prerelease，不接入自动更新也不标记 latest。 |
