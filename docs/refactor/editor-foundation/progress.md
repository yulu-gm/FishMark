...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates and reorder; one per-tab transaction shared by document IO, reorder, and cross-window transfer; persisted adapter `path: string` separated from nullable untitled projections; runtime-validated canonical Save/reload/close identity; no renderer-selected watcher tab/path; per-window canonical watcher application queue plus generation-checked per-webContents two-phase state queue whose stat I/O cannot block newer path state; explicit stale reload `success(snapshot) | revision-stale`; revision-bearing native-close generations; single-flight ready-gated detach; live sender-window validation; old service/dead facade/`getTabPath` deletion. Fresh adapter/watcher focus: 10 files/260 tests; tight focus 4 files/56 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 154-file/1,947-test suite; renderer/workspace-domain/Electron/CLI build and runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `17faea4` |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final blocker closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, removed reload's allow-stale projection path, made detach/registration/native close fail closed, separated persisted adapter documents from nullable untitled projections, and replaced watcher mutable async closures with a generation-checked per-webContents two-phase state machine. | Final adapter/watcher RED reproduced 12 direct boundary failures plus two stale callback/baseline interleavings. A final RED required B sync to finish while A stat remained deferred and proved both cases blocked in the first queue implementation. GREEN validates adapter success at runtime, locks ordinary Save/reload/close identity to captured checkpoints, runs stat outside the serial state queue, re-enters with generation/entry CAS, awaits pending sync before write admission, clears suppression on failure, contains callback rejection, and prevents destroyed sync from installing a watcher. Latest adapter/watcher focus is 10 files/260 tests; tight focus is 4 files/56 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 154 files/1,947 tests; renderer/workspace-domain/Electron/CLI build and emitted runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `17faea4`. |
