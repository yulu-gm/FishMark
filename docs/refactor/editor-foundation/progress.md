...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`, immutable `TextBuffer`/session/projections, revision-derived dirty state, exact saved checkpoint, pure workspace rules, main IPC mapper, captured guards, one main-owned per-document FIFO IO coordinator, native window-close multi-tab leases with post-confirm ordered-set validation held through unregister, ready-gated detach, and old service deletion. Fresh focused domain/main: 13 files/143 tests; renderer regressions: 4/177; editor-foundation: 7/259; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 147-file/1,860-test suite; renderer/Electron/CLI build; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; full-range diff/residue/deep-import checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, commits `387db42` through `a9390df` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to unchanged shared IPC DTOs. | Dirty is revision equality and durable equivalence uses an exact saved checkpoint. Main's one per-document FIFO coordinator serializes Save, Save As, reload, and individual close without delaying `updateDraft` or unrelated tabs. Native window close holds a stable multi-tab lease across the renderer handshake until unregister. Captured owner/revision/checkpoint still makes each completion fail closed. Detach instead waits with the tab in the source, revalidates source ownership at ready, and atomically moves the latest canonical session; no service facade or dual state remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Canonical text/revision/saved checkpoint, immutable projections, window/tab ownership, IPC mapping, one main-owned per-document FIFO IO coordinator, native window-close multi-tab leases with final ordered-set validation, ready-gated detach, rollback bounds, and error preservation now share one `WorkspaceState`; the old service/test and obsolete symbols are deleted. | RED first reproduced reload/save divergence and discard-followed-by-write; a later RED proved that a tab added during a positive close handshake escaped the initial lease. GREEN covers reload-first/save-first, individual close in both orderings, native confirmed/cancel close, tab-set changes during acquisition/handshake, multi-tab acquisition/FIFO/idempotent/error/independent-tab behavior, immutable domain rules, detach rollback, and stale edit fail-closed paths. Fresh focused domain/main 13 files/143 tests; renderer 4/177; editor-foundation 7/259; runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 147 files/1,860 tests; renderer/Electron/CLI build; formal behavior 121/121 cases and 2,541/2,541 targets with 79 existing/1,928 runner/534 known/0 unexpected/not-run in 25.874s. Full-range diff, obsolete-token, deep-import, and scoped wire checks passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` owns application ports/use cases and remains blocked on RF-101 `COMPLETE`. |
