...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; independent location/object ownership registries and typed ambiguous-owner detection; fail-closed filesystem identity resolution; one non-React renderer transaction application with canonical known/unknown admission, strict FIFO, generated per-tab outbox, exact `{tabId, epoch, loadRevision}` CodeMirror ownership, target/all-tab drains, tokenized sealing/sealed/releasing editor-transition acknowledgement, structural CodeMirror hard read-only, reload cutoff preservation, invocation-bound save transactions, and presentation-only hooks; typed application-owned editor test adapter; exact confirmation and main owner/active-tab/exact-window-instance CAS before focus; opaque active close-save lease capability; one `tab -> location -> object` transaction order shared by open/save/reload/close and transfer; typed reload stale/error results; stateless latest-intent watcher ownership; old main service, renderer hook transaction lanes, split save orchestration, raw test snapshot/mutation paths, direct snapshot-push event, rollback activation, duplicate close-save facade, legacy test dialog facade, and dead APIs deleted. | Sixth-round focused 13 files / 654 tests passed; build passed. Fresh managed-sandbox full run collected 163 files / 2,030 tests with 2,022 passed, 7 failed, 1 skipped; 2 code regressions were fixed and focused green, while the remaining 5 failures are Windows process-tree cleanup failures in 3 test-harness files. Full gate remains blocked pending escalated revalidation and is not recorded as PASS. | Pending root escalated full-gate revalidation, then independent architecture acceptance and task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through current RF-101 closure |
...
| 2026-07-16 / amended 2026-07-28 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Sixth review closure adds an exact CodeMirror-acknowledged sealing/sealed/releasing barrier around active destructive operations, structural hard read-only, and a typed application-owned editor test adapter; raw snapshot injection/mutation and dead renderer owner APIs are deleted. | RED reproduced pre-read-only destructive dispatch, late input loss, public/command-path read-only bypass, stale undo resurrection, transition release liveness gaps, and raw test-bridge state injection. GREEN waits for exact transition acknowledgement before final capture/drain/IPC, uses one private canonical replacement annotation with no history, releases into a new epoch, and drives tests through owned commands plus editor gestures. Latest renderer focus: 13 files / 654 tests. | Build passed. Managed-sandbox full run: 163 files / 2,030 tests, 2,022 passed, 7 failed, 1 skipped; 2 editor-history failures were fixed and focused green. The remaining 5 failures are Windows process-tree cleanup failures across 3 test-harness files, so full verification is blocked pending escalated revalidation and is not PASS. | Pending root escalated full-gate revalidation, then independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`; implementation closure is recorded in the task handoff. |
