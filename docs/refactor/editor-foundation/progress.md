...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`, one per-document FIFO IO coordinator, watcher-complete ordinary Save transactions, revision-bearing native-close confirmations, exact request-generation identity, active-confirmation drain under timeout/abort, ready-gated detach, and old service/dead save facade deletion. Fresh close/save/broker/main focus: 6 files/61 tests; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 148-file/1,875-test suite; renderer/Electron/CLI build; prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, commits `387db42` through `f1f74eb` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to unchanged shared IPC DTOs. | Main's per-document FIFO coordinator includes the full ordinary Save watcher lifecycle. Native close binds confirmation and COMPLETE to one `{ windowId, requestId }`; timeout/abort settles the request but the outer lease awaits active confirmation drain, preventing in-flight prompt/write overlap. Detach remains ready-gated; no service facade, dual state, or dead application Save API remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Canonical state, one per-document FIFO coordinator, full-watcher Save transactions, request-generation-bound native-close confirmation, active-confirmation drain, ready-gated detach, rollback bounds, and error preservation now share one `WorkspaceState`. | RED reproduced cross-generation confirmation risk and premature lease release after timeout/abort. GREEN covers broker generation/drain 10/10, close cancellation 16/16, shared coordinator prompt/write timeout integration 7/7, and affected focus 6 files/61 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 148 files/1,875 tests; renderer/Electron/CLI build. Prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. |
