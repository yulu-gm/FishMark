...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`, immutable `TextBuffer`/session/projections, revision-derived dirty state, exact saved checkpoint, pure workspace rules, main IPC mapper, captured owner/revision/checkpoint guards for Save, Save As, reload, and close, ready-gated detach with ready-time source-owner revalidation and latest-session move, and old service deletion. Fresh focused domain/main: 10 files/128 tests; renderer regressions: 4/177; editor-foundation: 7/259; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 144-file/1,845-test suite; renderer/Electron/CLI build; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; documentation diff check passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, commits `387db42` through `3fdc060` |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Canonical text/revision/saved checkpoint, immutable projections, window/tab ownership, IPC mapping, captured Save/Save As/reload/close guards, ready-gated detach, rollback bounds, and error preservation now share one main-owned `WorkspaceState`; the old service/test and obsolete symbols are deleted. | RED/GREEN and review fixes cover immutable buffer/session rules, equal-content reload, mismatched save rejection, workspace activation/move/detach rules, stale IO owner/revision races, detach source-owner revalidation and latest-session ready-time move, lifecycle rollback, error identity, and cleanup timeouts. Fresh focused domain/main 10 files/128 tests; renderer 4/177; editor-foundation 7/259; runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 144 files/1,845 tests; renderer/Electron/CLI build; formal behavior 121/121 cases and 2,541/2,541 targets with 79 existing/1,928 runner/534 known/0 unexpected/not-run in 25.929s. Zero obsolete tokens/deep imports and no Task 4 shared/preload/renderer diff. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` owns application ports/use cases and remains blocked on RF-101 `COMPLETE`. |
