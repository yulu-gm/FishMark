...
| RF-303 | Recovery journal and session restore | RF-302 | `IN_PROGRESS` | Added `recovery-journal.ts` (+7), `workspace-persistence.ts` (+8), `packages/workspace-application/src/recovery.ts` (+6), and `WorkspaceState.exportSnapshot()` (+1): append-only checksummed journal, validated deserialization, the recovery use case + journal port, and full session snapshot export. Remaining: domain `restoreSnapshot`, and startup/shutdown wiring in `main.ts`. | typecheck/test/build | — | — |
