...
| RF-303 | Recovery journal and session restore | RF-302 | `IN_PROGRESS` | Added `recovery-journal.ts` (+7), `workspace-persistence.ts` (+8), `packages/workspace-application/src/recovery.ts` (+6), `WorkspaceState.exportSnapshot()`/`restoreSnapshot()` (+2, round-trip verified). Remaining: startup/shutdown wiring in `main.ts` (journal accepted edits, load+restore on boot, mark-clean+prune on quit, corruption notification). | typecheck/test/build | — | — |
