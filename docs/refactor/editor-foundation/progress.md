...
**Current task:** `RF-303` — recovery journal complete; persistence/restore use case and startup/shutdown wiring remain

**Next required skill:** `$fishmark-task-execution` for `RF-303`
...
| M3 | Data safety and recovery | `IN_PROGRESS` | 2 | 4 | Inactive files protected; save/recovery/close are canonical |
...
**Program completion:** 10 / 38 tasks.
...
| RF-303 | Recovery journal and session restore | RF-302 | `IN_PROGRESS` | Added `src/main/infrastructure/recovery-journal.ts` (+ 7 tests): append-only JSONL journal, checksummed atomically-replaced snapshot, corrupt-file quarantine, truncate. Added `WorkspaceState.exportSnapshot()` (+ `WorkspaceSnapshot`/`WorkspaceSessionSnapshot` types, 1 test) capturing all windows/tabs/full session state. Remaining: `workspace-persistence.ts`, `packages/workspace-application/src/recovery.ts`, and startup/shutdown wiring. | typecheck/test/build | — | — |
...
There are no accepted external blockers or roadmap deviations. RF-001, RF-002, RF-101, RF-102, RF-201, RF-202, RF-203, RF-204, RF-301, and RF-302 are complete. M0, M1, and M2 are each `COMPLETE`; M3 is 2/4 `IN_PROGRESS`; accepted program completion remains 10/38. RF-303 is the next dependency-ready task.
