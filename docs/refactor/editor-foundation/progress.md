...
| 2026-07-16 / amended 2026-07-28 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Post-review closure adds exact CodeMirror-acknowledged sealing/sealed/releasing transitions, structural hard read-only, fresh-state canonical undo boundaries, recoverable epoch-only rebind, a bounded reload ambiguity checkpoint, disposed admission guards, and a typed application-owned editor test adapter; raw snapshot injection/mutation and dead renderer owner APIs are deleted. | RED reproduced pre-read-only dispatch, late input loss, empty/non-empty stale undo resurrection, recoverable history loss, response-lost reload draft loss with and without an outbox entry, disposal before/after canonical admission, inactive reload history reset, transition liveness gaps, and raw test-bridge state injection. GREEN waits for exact transition acknowledgement before final capture/drain/IPC, recreates state only for a real active canonical success, preserves history on cancel/stale/error, restores the sealed checkpoint on ambiguity, and settles disposed work without a new barrier. Latest renderer focus: 13 files / 667 tests. | Fresh typecheck, lint (0 errors / 8 existing warnings), build, and escalated full Vitest passed: 163 files / 2,042 passed plus 1 explicit skip out of 2,043 total. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`; implementation closure is recorded in the task handoff. |
