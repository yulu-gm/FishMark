...
| RF-304 | Main-owned conflict and close workflows | RF-303 | `IN_PROGRESS` | Added domain `acceptExternalDiskVersion` (+1 test): records the external disk version while keeping in-memory text (the "keep memory" decision foundation). Remaining: `resolve-external-change.ts` application use case, close-workspace flushEdits iteration, renderer client/conflict-banner migration, delete `useExternalConflictController.ts` + shell-state conflict reducers. | lint/typecheck/test/build/scenario | — | — |
