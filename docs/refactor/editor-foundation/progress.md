...
Production main sessions now store canonical Markdown in a private CodeMirror `Text` implementation behind the runtime-neutral domain `TextBuffer` port. Explicit factory injection preserves dependency direction; revisioned non-empty edit batches validate and apply original offsets atomically, advance once, and use contiguous per-client high-watermarks for idempotence. The current full-draft IPC path remains only as transport compatibility until RF-204 and does not become a second canonical owner.

### Verification

- Post-fix domain/infrastructure focus: 4 files / 163 tests; architecture focus: 1 file / 209 tests.
- Earlier cross-layer focus: 63 files / 816 passed + 1 existing skip; editor-foundation: 7 files / 285 tests.
- Lint passed with 0 errors / 8 existing warnings; typecheck passed.
- Full Vitest: 166 files / 2,162 passed + 1 explicit skip out of 2,163 total.
...
- Formal editor behavior: 121/121 cases, 2,541/2,541 targets, 79 existing, 1,928 runner, 534 known defects, 0 unexpected, 0 not-run in 24,614 ms.
- Independent specification/architecture acceptance: final `PASS`, P0/P1/P2 = 0, no open questions. Final quality review: Critical/Important/Minor = 0, `Ready: Yes`. Formal task acceptance: `PASS`.
