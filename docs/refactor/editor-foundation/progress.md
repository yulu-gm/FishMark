...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`, immutable `TextBuffer`/session/projections, revision-derived dirty state, exact saved checkpoint, pure workspace rules, main IPC mapper, captured guards, one main-owned per-document FIFO IO coordinator, watcher-complete ordinary Save transactions, revision-bearing native-close confirmations held through unregister, a bounded abortable close broker, ready-gated detach, and old service/dead save facade deletion. Fresh close/save/broker/main focus: 7 files/53 tests; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 148-file/1,866-test suite; renderer/Electron/CLI build; prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, commits `387db42` through `f63e5e6` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to unchanged shared IPC DTOs. | Dirty is revision equality and durable equivalence uses an exact saved checkpoint. Main's per-document FIFO coordinator includes the full ordinary Save watcher lifecycle. Native close carries a frozen ordered owner/revision confirmation through a 15-second abortable broker, revalidates it before native close, and holds the lease through unregister. Detach remains ready-gated; no service facade, dual state, or dead application Save API remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Canonical state, one per-document FIFO coordinator, full-watcher Save transactions, frozen revision-bearing native-close confirmation, bounded abortable request ownership, ready-gated detach, rollback bounds, and error preservation now share one `WorkspaceState`; the old service and duplicate application Save API are deleted. | RED reproduced reload/save divergence, discard-followed-by-write, overlapping watcher transactions, late post-CONFIRM revisions, and unbounded close requests. GREEN covers the close/save/broker/main focus at 7 files/53 tests, including exact-once timeout/abort cleanup and real watcher behavior. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 148 files/1,866 tests; renderer/Electron/CLI build. Prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` owns application ports/use cases and remains blocked on RF-101 `COMPLETE`. |
