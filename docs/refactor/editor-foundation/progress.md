...
| --- | --- | --- | --- | --- | --- | --- | --- |
| RF-001 | Editing behavior baseline | — | `DEV_DONE` | Hardened runner/observer/protocol/launcher: 10 focused files / 74 tests; harness + driver: 26 files / 150 tests; real Typora catalog/capture validation passed 14/14; public scenario passed 121/121 steps in one batch; launcher, adapter, and public-CLI timeout/abort/nonzero process-tree boundaries passed. | Formal Electron gate: 121/121 cases, 2,541 targets, 79 existing + 1,928 runner matches, 534 exact known defects, 0 unexpected, 0 not-run, 25.899s. Deterministically hashed explicit sets cover 2,541/2,541 with 0 gaps. Editing experience: 79/79 passed. lint 0 errors (8 pre-existing warnings); typecheck, 134-file/1,472-test suite, and build passed. | Architecture/task acceptance pending | `codex/editor-foundation-refactor` |
...
| 2026-07-14 | RF-001 | Added the single-window Electron manifest runner and hardened it with line-local observation, aspect-mapped values, deterministic calibration identity, catalog-driven Typora conversion, calibration-independent raw cases, and shared cross-platform process-tree termination; corrected top-level Shift+Tab contract without changing product runtime behavior. | Hardened focused set 10 files / 74 tests; harness + driver 26 files / 150 tests; Typora conversion 14/14; real launcher/adapter/public-CLI timeout, abort, nonzero, and root-exited descendant boundaries; public scenario 121/121 one batch. | Formal runner 121/121 and 2,541/2,541 in 25.899s with 79 existing/1,928 runner/534 known/0 unexpected/0 not-run; legacy 79/79; lint/typecheck/134-file 1,472-test suite/build passed. | Pending | Raw JSON remains ignored; calibration `fnv1a32-ae988a59` persists 2,007 explicit verified targets and 534 exact typed defect observations. |

## 8. Blockers and deviations

There are no accepted external blockers or roadmap deviations. RF-001 has no remaining implementation gap; its explicit 2,007 verified targets and 534 exact target mismatches cover the whole contract, and the mismatches are recorded current product defects for later roadmap tasks rather than unexecuted evidence. Formal architecture and task acceptance remain pending.
