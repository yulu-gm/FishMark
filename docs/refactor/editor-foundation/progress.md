...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates; one per-tab transaction shared by document IO and cross-window transfer; explicit stale Save/Save As/reload failure semantics; watcher-complete Save; revision-bearing native-close generations with transport timeout, active-confirmation drain, and post-confirm missing-COMPLETE watchdog; ready-gated detach; old service/dead facade deletion. Fresh architecture-fix focus: 14 files/142 tests; real shared-coordinator integration: 12/12; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 151-file/1,899-test suite; renderer/workspace-domain/Electron/CLI build; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `d5ee7f0` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to shared workspace snapshot DTOs. | Draft mutation is a sender-derived owner compare-and-set; stale renderers neither mutate nor receive the target owner's projection. One per-tab coordinator owns Save/Save As/reload/close and cross-window move/detach transfer, with ownership revalidated inside the lease and detach readiness awaited outside it. Owner/missing completion results fail explicitly. Native close carries one `requestId` through REQUEST, renderer draft flush, CONFIRM, and COMPLETE: transport is bounded before CONFIRM, active native UI has no timer, and a separate watchdog fails closed after the scope drains if COMPLETE never arrives. No service facade, parameterless close compatibility path, dual state, or dead application Save API remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final architecture closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, made stale application mutations explicit failures, and added a post-confirm liveness bound without timing active native UI. | Initial RED reproduced stale source mutation/projection leakage, uncoordinated owner transfer, false-success Save/Save As/reload, and missing-COMPLETE lease retention. GREEN proves real move and detach reject late source drafts without changing target content/revision; all four Save/move/reload/detach transaction orderings; total mutation-result handling; transport/active/post-confirm close phases; and 14 files/142 focused tests plus 12/12 shared-coordinator integration tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 151 files/1,899 tests; renderer/workspace-domain/Electron/CLI build; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `d5ee7f0`. |
