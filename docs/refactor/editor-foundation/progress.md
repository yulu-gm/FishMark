...
| RF-002 | Architecture and performance guards | RF-001 | `DEV_DONE` | Architecture guard 145/145; analyzer 48/48; provenance plugin 7/7; final `perf:baseline` 7 files/221 tests. The manifest owns 23 bundle checks and 73 exact CodeMirror targets; real micromark scan counters, case-folded boundaries, transparent AST wrappers and schema-v1 Vite module provenance close the reviewed evidence gaps. | lint 0 errors/8 pre-existing warnings; typecheck; 138-file/1,706-test suite; renderer/Electron/CLI build; formal behavior 121/121 cases and 2,541/2,541 targets with 79 existing + 1,928 runner + 534 known + 0 unexpected/not-run; ordinary dist 0 maps/0 provenance; diff check all passed. | Earlier architecture `FAIL` at `869b9f9`, `b870a36` and `fe26b6a`; all findings plus the final evidence-completeness review are repaired. Architecture re-review, then task acceptance, remains required; `docs/plans/2026-07-14-rf-002-handoff.md` | `codex/editor-foundation-refactor` |
...
| 2026-07-15 | RF-002 | Final architecture closure added transparent-wrapper/computed-property micromark detection, reverse-unique active package rules and case-folded Windows boundaries; replaced inferred parse-entry counts with explicit micromark document-scan instrumentation; deleted subordinate formatters; and replaced Source Map text-coverage/identity-map assumptions with schema-v1 two-phase Vite provenance. Provenance `moduleIds` are the only forbidden source-group authority; emitted maps are integrity validated and attested mapless chunks are `NOT_EMITTED`. | Architecture RED 145/131 pass + 14 expected failures -> GREEN 145/145. Instrumentation RED 57/50 pass + 7 expected failures -> GREEN 57/57. Provenance RED accepted missing provenance -> GREEN analyzer 48/48 plus plugin 7/7. Real `perf:baseline` passed the canonical 23 checks and 7 files/221 tests; exact full scans are open 504, edit 502, selection 0, ordered-list 503, outline 502, metrics 503. | lint 0 errors/8 pre-existing warnings; typecheck; 138 files/1,706 tests; renderer/Electron/CLI build; final formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; ordinary dist 0 maps/0 provenance; diff check PASS. | Architecture re-review, then task acceptance | M0 remains 1/2 and program 1/38. No editor behavior, Markdown fixture identity or roadmap ordering changed; no identity-map/compatibility code remains. Two initial full behavior attempts showed focus-derived visibility-only observation flakes; three isolated repeats per affected case and two fresh full runs passed. A separate runner-hardening task should add condition-based focus/decoration settlement and diagnostics without changing calibration. |
