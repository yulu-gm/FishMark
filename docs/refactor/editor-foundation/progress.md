...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates and reorder; one per-tab transaction shared by document IO, reorder, and cross-window transfer; explicit stale Save/Save As failures plus hard-cut reload `success(snapshot) | revision-stale`; stale reload retains conflict protections and does not record recent; watcher-complete Save; revision-bearing native-close generations with transport timeout, active-confirmation drain, and post-confirm missing-COMPLETE watchdog; single-flight ready-gated detach whose concurrent callers cannot register before transfer and whose queued timeout/close/load-fail cannot create ghost domain/main windows; live sender-window validation; old service/dead facade deletion. Fresh ownership-race focus: 10 files/127 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 153-file/1,923-test suite; renderer/workspace-domain/Electron/CLI build; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `f6ae3e6` |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final blocker closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, removed reload's allow-stale projection path in favor of a hard-cut discriminated result, made queued detach cancellation and live target-window registration fail closed through one executable pre/post-ready owner transaction, and retained the post-confirm liveness bound without timing active native UI. | RED reproduced stale reload success, ready resolving after queued detach cancellation, and the absence of an executable live-window registration boundary. GREEN proves the exact reload union, fail-closed detach/ready behavior, and sender/owner object/ID revalidation around ready with zero registration side effects on every invalid lifecycle; latest expanded focus is 10 files/102 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 152 files/1,915 tests; renderer/workspace-domain/Electron/CLI build and emitted runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `1e7baed`. |
