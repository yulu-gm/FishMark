...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to unchanged shared IPC DTOs. | Dirty is revision equality, durable equivalence uses an exact saved checkpoint, asynchronous completion is owner/revision-bound and fail-closed, and no service facade or dual state remains. |
