...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`, one per-document FIFO IO coordinator, watcher-complete ordinary Save transactions, revision-bearing native-close confirmations, end-to-end request identity, one confirmation per generation, transport-only timeout, active-confirmation drain after renderer loss, ready-gated detach, and old service/dead save facade deletion. Fresh affected focus: 9 files/234 tests; main/broker/integration close focus: 4 files/29 tests; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 149-file/1,878-test suite; renderer/Electron/CLI build; prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, commits `387db42` through `7a50095` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to shared workspace snapshot DTOs. | Main's per-document FIFO coordinator includes the full ordinary Save watcher lifecycle. Native close carries one `requestId` through REQUEST, renderer draft flush, CONFIRM, and COMPLETE. The 15-second bound covers only transport up to the first exact CONFIRM; native prompt/Save As may continue without that timeout, while renderer loss settles fail-closed and the outer lease awaits active confirmation drain. Detach remains ready-gated; no service facade, parameterless close compatibility path, dual state, or dead application Save API remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Canonical state, one per-document FIFO coordinator, full-watcher Save transactions, end-to-end request-generation-bound native-close confirmation, transport-only timeout, active-confirmation drain, ready-gated detach, rollback bounds, and error preservation now share one `WorkspaceState`. | RED reproduced cross-generation confirmation risk, duplicate-confirm side effects, prompt cancellation by the transport timer, and premature lease release after renderer loss. GREEN covers exact REQUEST-to-CONFIRM identity, one confirmation per generation, a prompt continuing beyond 60 seconds after transport acceptance, renderer abort/window-close fail-closed drain, and affected focus 9 files/234 tests (main/broker/integration close focus 4 files/29 tests). | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 149 files/1,878 tests; renderer/Electron/CLI build. Prior formal behavior remains 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. |
