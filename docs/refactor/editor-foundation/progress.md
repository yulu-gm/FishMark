...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final blocker closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, removed reload's allow-stale projection path, made detach/registration/native close fail closed, separated persisted adapter documents from nullable untitled projections, and replaced watcher mutable async closures with a generation-checked per-webContents state machine. | Final adapter/watcher RED reproduced 12 direct boundary failures plus two strengthened stale callback/baseline interleavings. GREEN validates adapter success at runtime, locks ordinary Save/reload/close identity to captured checkpoints, awaits begin-write admission, serializes watcher infrastructure transitions per sender, clears suppression on failure, contains callback rejection, and prevents destroyed sync from installing a watcher. Latest adapter/watcher focus is 10 files/259 tests; tight focus is 4 files/55 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 154 files/1,946 tests; renderer/workspace-domain/Electron/CLI build and emitted runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `26a7e22`. |
