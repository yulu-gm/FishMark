...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates; one per-tab transaction shared by document IO and cross-window transfer; explicit stale Save/Save As failures plus hard-cut reload `success(snapshot) | revision-stale`; stale reload retains conflict protections and does not record recent; watcher-complete Save; revision-bearing native-close generations with transport timeout, active-confirmation drain, and post-confirm missing-COMPLETE watchdog; ready-gated detach whose queued timeout/close/load-fail cannot create ghost domain/main windows; live sender-window validation; old service/dead facade deletion. Fresh blocker-fix focus: 20 files/361 tests; emitted Electron runtime verifier passed. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 151-file/1,904-test suite; renderer/workspace-domain/Electron/CLI build; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `b331daa` |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final blocker closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, removed reload's allow-stale projection path in favor of a hard-cut discriminated result, made queued detach cancellation and live target-window registration fail closed, and retained the post-confirm liveness bound without timing active native UI. | RED reproduced stale reload reporting success/clearing external conflict and ready resolving after queued detach cancellation. GREEN proves `revision-stale` carries no snapshot/recent side effect and keeps autosave/manual-save conflict protection; timeout/close/load-fail reject both detach and ready with source/last-focus/domain/binding unchanged. Owner CAS, shared transfer orderings, and transport/active/post-confirm close phases remain green; blocker focus is 20 files/361 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 151 files/1,904 tests; renderer/workspace-domain/Electron/CLI build and emitted runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `b331daa`. |
