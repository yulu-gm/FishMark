...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates and reorder; one per-tab transaction shared by document IO, reorder, and cross-window transfer; canonical-only file identity for Save/Save As/reload; no renderer-selected watcher tab/path; per-window canonical watcher queue; explicit stale reload `success(snapshot) | revision-stale`; revision-bearing native-close generations; single-flight ready-gated detach; live sender-window validation; old service/dead facade/`getTabPath` deletion. Fresh file-identity/watcher focus: 14 files/269 tests. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 154-file/1,929-test suite; renderer/workspace-domain/Electron/CLI build; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `95ef4eb` |
