...
| RF-101 | Extract workspace domain | RF-002 | `DEV_DONE` | Production `@fishmark/workspace-domain`; sender-derived owner CAS for draft updates and reorder; one per-tab transaction shared by document IO, reorder, and cross-window transfer; persisted adapter `path: string` separated from nullable untitled projections; runtime-validated canonical Save/reload/close identity; no renderer-selected watcher tab/path; stateless canonical watcher application plus per-webContents latest-intent/CAS infrastructure state owner with superseding sync admission, exact entry/observation/write identities, own-write callback silence, safe lifecycle rollback, and no long-I/O queue; explicit stale reload `success(snapshot) | revision-stale`; revision-bearing native-close generations; single-flight ready-gated detach; live sender-window validation; old service/dead facade/`getTabPath` deletion. | Fresh watcher/application focus: 2 files/26 tests; watcher/file-operation/close focus: 7/92; lint 0 errors/8 pre-existing warnings; typecheck; 154-file/1,958-test suite; renderer/workspace-domain/Electron/CLI build and runtime verifier; editor-foundation 7/259; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run; diff/residue checks passed. | Pending independent architecture acceptance, then task acceptance; execution handoff: `docs/plans/2026-07-16-rf-101-handoff.md` | `codex/editor-foundation-refactor`, implementation commits `387db42` through `085b51f` |
...
| 2026-07-16 | `@fishmark/workspace-domain` owns the single canonical workspace/session state; main owns its only live instance and maps immutable projections to shared workspace snapshot DTOs. | Draft mutation is a sender-derived owner compare-and-set; stale renderers neither mutate nor receive the target owner's projection. One per-tab coordinator owns Save/Save As/reload/close and cross-window move/detach transfer, with ownership revalidated inside the lease and detach readiness awaited outside it. Owner/missing completion results fail explicitly; reload revision staleness uses a distinct no-projection result and preserves external-conflict protection. Queued detach lifecycle cancellation rejects ready and cannot create a ghost window; main uses no sender-ID fallback and validates the live owner window around ready. Native close carries one `requestId` through REQUEST, renderer draft flush, CONFIRM, and COMPLETE: transport is bounded before CONFIRM, active native UI has no timer, and a separate watchdog fails closed after the scope drains if COMPLETE never arrives. No service facade, reload allow-stale path, parameterless close compatibility path, dual state, or dead application Save API remains. |
...
| 2026-07-16 | RF-101 | Added the production runtime-neutral workspace-domain package and hard-cut main from the old service. Final blocker closure made draft updates owner-aware, put move/detach ownership transfer under the same per-tab transaction as document IO, removed reload's allow-stale projection path, made detach/registration/native close fail closed, separated persisted adapter documents from nullable untitled projections, and replaced watcher Promise queues with one per-webContents latest-intent/CAS state owner. | The latest watcher RED reproduced 9 deterministic failures across same-entry callback reversal, two own-write callback/completion reversals, stale-sync begin/application head-of-line blocking, watch creation/old-close/new-rollback failures, and destroy close exceptions. GREEN makes the application stateless, runs every stat outside state mutation, commits only exact latest identities, lets superseding sync admission wake begin-write, makes callbacks captured during own writes silent, and tombstones/deletes before safe close. Fresh watcher/application focus is 2 files/26 tests; watcher/file-operation/close focus is 7/92. | Fresh lint 0 errors/8 pre-existing warnings; typecheck; 154 files/1,958 tests; renderer/workspace-domain/Electron/CLI build and emitted runtime verifier; editor-foundation 7 files/259 tests; formal behavior 121/121 cases and 2,541/2,541 targets with 0 unexpected/not-run. Diff and obsolete-token scans passed. | Pending independent architecture and task acceptance; no verdict recorded by execution. | M1 remains 0/2 complete and program remains 2/38 until acceptance. `RF-102` remains blocked on RF-101 `COMPLETE`. Implementation head `085b51f`. |
