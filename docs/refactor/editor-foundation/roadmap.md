...
2026-07-28 closure: main now resolves separate canonical-location and filesystem-object identities (`dev + ino` when reliable, canonical-path fallback only when object IDs are explicitly unsupported or unreliable), and the domain enforces one editable session through both registries across all windows. Open resolves a prospective canonical location before taking its location lease, then resolves the existing object inside the lease. Duplicate open releases file locks, takes the owner tab lease, revalidates `tab -> location -> object`, records recency, and only then activates; cross-window activation sends a typed owner snapshot after watch sync and before focus, without projecting foreign state to the source renderer. Save/Save As/reload use the same lock order, mixed location/object conflicts are checked independently, and native close-save can enter held-lease IO only with an opaque, active coordinator capability. Reload identity/read/conflict failures are typed across main/preload/renderer instead of leaking IPC rejection text. Hard-link aliases, prospective-to-existing identity migration, open/close or open/move races, and mixed-index migration cannot create two clean sessions. Status remains `DEV_DONE` until independent acceptance.
