...
2026-07-28 closure: main now resolves separate canonical-location and filesystem-object identities (`dev + ino` when reliable, canonical-path fallback only when object IDs are explicitly unsupported or unreliable), and the domain enforces one editable session through both registries across all windows. Owner lookup is a total `none | owned | ambiguous` result, so a location/object split across different tabs fails closed. Open resolves a prospective canonical location before taking its location lease, then resolves the existing object inside the lease. Duplicate open validates `tab -> location -> object`, releases every lease, and sends a bounded typed activation request to the owner renderer. Renderer workspace snapshot writers now share one strict per-window FIFO coordinator; non-replaceable open/create/close/reorder/detach operations are never dropped, while activation alone has latest-intent supersession. A per-tab draft outbox binds every editor callback to the tab active at callback time and clears an entry only on an exact content acknowledgement. Every dispatched response records canonical main state before the next queued operation; unknown activation transport outcomes explicitly fetch the canonical snapshot. Once main has confirmed a target activation, draft-drain failure no longer dispatches a competing rollback activation: renderer keeps the canonical target visible and retains the source outbox for a later retry. An already-canonical target flushes its outbox without redundant main activation. Only after positive confirmation does main reacquire the owner tab lease, revalidate typed ownership, the exact `BrowserWindow` object, and `activeTabId === requested tab`, then focus. Negative confirmation retries only if ownership moved/closed/became ambiguous; an unchanged owner fails without replaying activation. Timeout, renderer destruction, activation failure, owner move/close, a newer tab intent, and bounded retry budgets cannot report false `focused-existing`, and no foreign projection reaches the source renderer. Save/Save As/reload use the same lock order, mixed location/object conflicts are checked independently, and native close-save can enter held-lease IO only with an opaque, active coordinator capability. Reload identity/read/conflict failures are typed across main/preload/renderer instead of leaking IPC rejection text. Hard-link aliases, prospective-missing-to-existing materialization, open/close or open/move races, mixed-index migration, late draft responses, and save-refresh/activation interleavings cannot create two renderer truths. Status remains `DEV_DONE` until independent acceptance.
