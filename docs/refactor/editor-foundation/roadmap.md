...
**Parity baseline measured ahead of this task:** replaying every `fixtures/editor-behavior` case's `checkpoints[0].actions` through the pure planners and diffing source+selection against the recorded result matches 71 of 121 primary checkpoints. The gap is semantic rather than structural and is concentrated in `Enter` (22 mismatches) and `Tab`/`Shift+Tab` (9 mismatches, 13 actions no planner claims), while `Backspace` (2), `ArrowUp` (1), `ArrowDown` (2) and `InsertText` (1) are near parity and selection-only actions match exactly. The dominant missing rule is the legacy blank-line separator: a paragraph break writes `\n\n`, a break inside a blockquote writes `\n` + bare `>` separator + `\n` + quote prefix (`> alpha` + Enter must give `> alpha\n>\n> `, not `> alpha\n> `), and a list item keeps its own item prefix and numbering. Navigation and Backspace treat those separators as structural — arrows skip them to the next content line and Backspace at a trailing empty quote line deletes that line. Close these before switching routes, or `src/renderer/code-editor.test.ts` regresses on real behaviour.
