...
2026-07-28 closure: main now resolves separate canonical-location and filesystem-object identities (`dev + ino` when reliable, canonical-path fallback only when object IDs are explicitly unsupported or unreliable), and the domain enforces one editable session through both registries across all windows. Owner lookup is a total `none | owned | ambiguous` result, so a location/object split across different tabs fails closed. Open resolves a prospective canonical location before taking its location lease, then resolves the existing object inside the lease. Duplicate open validates `tab -> location -> object`, releases every lease, and sends a bounded typed activation request to the owner renderer. The owner renderer reuses one activation transaction: it captures the source tab, performs a bounded initial draft flush, awaits owner-checked activation IPC without applying the target snapshot, drains any source edits produced during that await through draft sync, then atomically checks and applies the latest target snapshot before confirming. A bounded drain failure activates the source tab again and preserves its latest local draft; an unmounted or superseded generation neither applies nor confirms success. An already-active target flushes pending draft state but performs no main activation mutation. Only after positive confirmation does main reacquire the owner tab lease, revalidate typed ownership, live window identity, and `activeTabId === requested tab`, then focus. Negative confirmation retries only if ownership moved/closed/became ambiguous; an unchanged owner fails without replaying activation. Timeout, renderer destruction, activation failure, owner move/close, a newer tab intent, and bounded retry budgets cannot report false `focused-existing`, and no foreign projection reaches the source renderer. Save/Save As/reload use the same lock order, mixed location/object conflicts are checked independently, and native close-save can enter held-lease IO only with an opaque, active coordinator capability. Reload identity/read/conflict failures are typed across main/preload/renderer instead of leaking IPC rejection text. Hard-link aliases, prospective-missing-to-existing materialization, open/close or open/move races, and mixed-index migration cannot create two clean sessions. Status remains `DEV_DONE` until independent acceptance.
