...
- [x] Serialize watcher sync, filesystem callbacks, begin-write, and complete-write per `webContents`; advance path generations at intent time, revalidate entry identity after every await, clear failed completion suppression, contain callback failures, and prevent destroyed watchers from being resurrected.
