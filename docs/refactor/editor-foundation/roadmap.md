...
- [x] Serialize watcher state transitions for sync, filesystem callbacks, begin-write, and complete-write per `webContents` without holding the queue across stat I/O; assign every sync intent its own token while retaining separate path-entry generations, re-enter with the exact sync token plus entry/path identity checks after every await, await the latest pending sync before write admission, clear failed completion suppression, contain callback failures, and prevent destroyed watchers from being resurrected.
