...
- [x] Serialize watcher state transitions for sync, filesystem callbacks, begin-write, and complete-write per `webContents` without holding the queue across stat I/O; advance path generations at intent time, re-enter with entry identity checks after every await, await pending sync before write admission, clear failed completion suppression, contain callback failures, and prevent destroyed watchers from being resurrected.
