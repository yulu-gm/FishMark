...
截至 2026-04-15，仓库已完成 `TASK-001`、`TASK-002` 和 `TASK-003`：当前已经具备可运行开发壳、目录边界，以及“通过安全 bridge 打开 UTF-8 Markdown 文件并把内容加载到临时 textarea 编辑面”的最小闭环。但保存、CodeMirror 6 接入和块级 Markdown 渲染仍未实现，因此当前人工验收应覆盖“项目骨架是否成立”和“打开 Markdown 文件最小闭环是否成立”，不要按完整编辑器功能去验。
...
3. 确认页面显示 `Yulora` 和 `TASK-003`
4. 确认页面文案明确写着当前可通过安全 Electron bridge 打开本地 UTF-8 Markdown 文件
5. 确认页面能显示 preload 暴露的 `platform` 字段
6. 点击 `Open Markdown`，选择一个 UTF-8 编码的 `.md` 文件
7. 确认页面显示当前文档名、完整路径和文件文本内容
8. 确认文本被加载到临时 `<textarea>` 中，并可在内存中编辑
9. 关闭并重新启动，确认开发壳仍能正常启动
